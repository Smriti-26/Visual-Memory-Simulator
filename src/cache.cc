...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        if ((cache_type == IS_LLC) && (way == LLC_WAY)) { // this is a bypass that does not fill the LLC
            ...
            if( NAME == "L1D")
            {
              string s = to_string(set)+" "+to_string(way);  
              ...
              cout<<"L1D access "<<sim_access[fill_cpu][0]+sim_access[fill_cpu][1]<<" set "<<set<<" way "<<way<<" miss "<<sim_miss[fill_cpu][0]+sim_miss[fill_cpu][1]<<" address "<<MSHR.entry[mshr_index].address<<endl;
            }
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
        ...
        if (way >= 0) { // writeback hit (or RFO hit for L1D)
            ...
            if( NAME == "L1D")
            {
              ...
              cout<<"L1D access "<<sim_access[writeback_cpu][0]+sim_access[writeback_cpu][1]<<" set "<<set<<" way "<<way<<" hit "<<sim_hit[writeback_cpu][0]+sim_hit[writeback_cpu][1]<<" address "<<WQ.entry[index].address<<endl;
            }
		  ...
		  if (cache_type == IS_L1I)
			...
			cpu = 0;
                    ...
                    if( NAME == "L1D")
                    {
                      ...
                      cout<<"L1D access "<<sim_access[writeback_cpu][0]+sim_access[writeback_cpu][1]<<" set "<<set<<" way "<<way<<" miss "<<sim_miss[writeback_cpu][0]+sim_miss[writeback_cpu][1]<<" address "<<WQ.entry[index].address<<endl;
                    }
		...
		if (RQ.entry[index].type == LOAD) {
			...
			cpu = 0;
                ...
                if( NAME == "L1D")
                {
                  ...
                  cout<<"L1D access "<<sim_access[read_cpu][0]+sim_access[read_cpu][1]<<" set "<<set<<" way "<<way<<" hit "<<sim_hit[read_cpu][0]+sim_hit[read_cpu][1]<<" address "<<RQ.entry[index].address<<endl;
                }
        ...
        if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0)) {
            ...
            if (way >= 0) { // prefetch hit
                ...
                if( NAME == "L1D")
                {
                  ...
                  cout<<"L1D access "<<sim_access[prefetch_cpu][0]+sim_access[prefetch_cpu][1]<<" set "<<set<<" way "<<way<<" hit "<<sim_hit[prefetch_cpu][0]+sim_hit[prefetch_cpu][1]<<" address "<<PQ.entry[index].address<<endl;
                }
...
{
      access_count++;
    if(access_count % 2000000 == 0)
    {
      i_set = (i_set + 15);
    }

    // uint32_t set = (get_set(packet->address) + i_set)%NUM_SET;
